...
\abstract{Terrain following coordinates are widely used in operational models.
More recently, the cut cell method has been proposed as an alternative that can more accurately represent orography.  In order to compare the accuracy of the different grids, it is typically necessary to use different models.  Instead, we use a single model to enable a like-for-like comparison of terrain following and cut cell grids.
...
The results of the tracer advection tests demonstrate that tracer accuracy depends upon alignment of the flow with the grid.  In the gravity waves test, results on all grids are in good agreement with existing results from the literature.  However, we find that the Lorenz computational mode is manifested as a vertical zig--zag in potential temperature only on the cut cell grid.
...
Here we describe the formulation of the terrain following grids and the method of cut cell grid construction.  The techniques presented here are used to define the grids for the experiments in the subsequent section.
...
\begin{align}
	h(x) &= \left\{ \begin{array}{l l}
	h_0 \cos^2 ( \alpha x ) \cos^2 ( \beta x ) & \quad \text{if $| x | < a$} \\
...
We conclude from this test that accuracy depends upon alignment of the flow with the grid, and accuracy is not significantly reduced by grid distortions.  Accuracy on the BTF grid is comparable with accuracy on the SLEVE grid in the horizontal advection test.
