...
The test was executed on the BTF, SLEVE and cut cell grids, and on a regular grid with flat terrain using a centred linear scheme and the upwind-biased cubic scheme.  Tracer contours at \(t = \SI{0}{\second}, \SI{5000}{\second}\) and \(\SI{10000}{\second}\) are shown in Figure~\ref{fig:advection-tracer}. 
...
By \(t = \SI{10000}{\second}\), the tracer suffers from diffusion and distortion on the BTF grid using the centred linear scheme.  Some artefacts remain above the mountain peak, and the computational mode is seen as a grid-scale oscillation that travels in the opposite direction to the wind.
...
Tracer minima and maxima are lower than values presented by \citet{schaer2002} because we do not interpolate the geometric height of cell faces from adjacent cell centres in order to calculate the streamfunction, \(\Psi(z)\).  Removing the interpolation from Sch\"{a}r's code results in minima and maxima that are equal to those obtained from the OpenFOAM advection solver presented here.
...
\begin{table*}[t]
	\caption{Minimum and maximum tracer magnitudes and \(\ell^2\) error norms at \(t = \SI{10000}{\second}\) in the horizontal tracer advection tests using centred linear and cubic upwind-biased schemes, and the fourth order scheme from \citet{schaer2002}.}
\label{tab:advection}
%
\centering
\footnotesize
\begin{tabular}{ r @{\hspace{1.0em}} l @{\hspace{1.0em}} l @{\hspace{1.0em}} l l @{\hspace{1.0em}} l @{\hspace{1.0em}} l}
\hline\hline
& \multicolumn{3}{c}{Centred linear} & \multicolumn{3}{c}{Cubic upwind-biased} \\
& \(\ell^2\) error & min & max & \(\ell^2\) error & min & max \\
\hline
Analytic   & 0 & 0 & 1 & 0 & 0 & 1 \\
% 	     l2		     min	     max	   l2		    min		     max        
BTF 	   & \num{0.0210}  & \num{-0.275}  & \num{0.925} & \num{0.00791}  & \num{-0.0446}  & \num{0.925} \\
SLEVE      & \num{0.00233} & \num{-0.0252} & \num{0.985} & \num{0.00108}  & \num{-0.0106}  & \num{0.982} \\
Cut cell   & \num{0.00224} & \num{-0.0251} & \num{0.985} & \num{0.000577} & \num{-0.00674} & \num{0.983} \\
No terrain & \num{0.00223} & \num{-0.0251} & \num{0.985} & \num{0.000576} & \num{-0.00674} & \num{0.983} \\
...
\begin{table*}[t]
	\caption{Minimum and maximum tracer magnitudes and \(\ell^2\) error norms at \(t = \SI{10000}{\second}\) in the terrain following tracer advection test using centred linear and cubic upwind-biased schemes.}
\label{tab:advection}
%
\centering
\footnotesize
