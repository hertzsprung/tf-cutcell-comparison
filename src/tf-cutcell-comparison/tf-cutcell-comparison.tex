...
% TABLES
\begin{table*}
	\caption{Minimum and maximum tracer magnitudes and \(\ell^2\) error norms (defined by equation~\ref{eqn:l2-error}) at \(t = \SI{10000}{\second}\) in the horizontal and terrain following tracer advection tests using centred linear and cubic upwind-biased schemes.  For the horizontal advection test, minimum and maximum values are given for the fourth order scheme using the modified code from \citet{schaer2002}.  \(\ell^2\) error norms were not calculated by this code.}
\label{tab:advection}
%
\centering
\footnotesize
