...
Tests were performed with two different stability profiles, both having an initial potential temperature field with $\theta(z = 0) = \SI{288}{\kelvin}$ and a constant static stability with Brunt-V\"ais\"al\"a frequency $N = \SI{0.01}{\per\second}$ everywhere, except for a more stable layer of $N = \SI{0.02}{\per\second}$.  Figure~\ref{fig:resting}a shows where this inversion layer is positioned in the two tests: the `high inversion' test follows \citet{klemp2011}, placing the layer between $\SI{2}{\kilo\meter} \leq z \leq \SI{3}{\kilo\meter}$; the `low inversion' test is designed to challenge the numerics on the cut cell grid by placing the inversion layer between $\SI{0.5}{\kilo\meter} \leq z \leq \SI{1.5}{\kilo\meter}$ so that it intersects the terrain.
...
The tests were integrated forward by 5 hours using a timestep $\Delta t = \SI{100}{\second}$ on the BTF, SLEVE and cut cell grids, and a regular grid with flat terrain.
The results presented in figure~\ref{fig:resting}b, which use a curl-free pressure gradient, have maximum spurious values of $w$ of \SI{0.37}{\meter\per\second} on the BTF grid with the high inversion layer, compared with a maximum of \(\sim \SI{7}{\meter\per\second}\) found by \citet{klemp2011} using their improved horizontal pressure gradient formulation.
Unlike the result from \citet{klemp2011}, the SLEVE grid does not significantly reduce vertical velocities compared to the BTF grid.
For the high inversion test, errors are two orders of magnitude smaller on the cut cell grid with vertical velocities of \(\sim \SI{1e-4}{\meter\per\second}\), but this advantage is lost when the inversion layer is lowered to intersect the terrain.
The smallest error of \(\sim \SI{1e-10}{\meter\per\second}\) is found on the regular grid.
The results for the high inversion test in figure~\ref{fig:resting}b have maximum errors that are comparable with \citet{weller-shahrokhi2014} but, due to the more stable split into implicitly and explicitly treated terms (described in the appendix), the errors decay over time due to the dissipative nature of the advection scheme.
...
In summary, spurious velocities in the resting atmosphere test were similar on terrain following and cut cell grids, with lower errors compared to those from \citet{klemp2011}.
\TODO{we previously concluded, ``The maximum vertical velocity was significantly decreased on the cut cell grid, so we conclude that non-orthogonality, or lack of alignment of the grid with surfaces of constant gravitational potential are a significant cause of numerical error in this test.'' --- but what can we say now we have the high/low inversion tests?}
