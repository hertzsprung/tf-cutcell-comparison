...
The second model is a modified version of the linear advection finite difference model first used by \citet{schaer2002}.  It uses terrain following coordinates and it is configured with leapfrog timestepping and a fourth-order centred difference scheme: \TODO{this wasn't properly documented in schaer2002 but we can document it using the Fortran code}
\begin{align}
	TODO
\end{align}
...
\TODO{Schaer defines u=-dPsi/dz, w=dPsi/dx --- check all this streamfunction stuff AGAIN}

The discrete velocity field is defined using a streamfunction, \(\Psi\).  Since \(u = \partial \Psi / \partial z\), the streamfunction is found by vertical integration of the velocity profile:
\begin{align}
