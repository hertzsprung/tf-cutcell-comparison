...
Following \citet{melvin2010}, the domain is \SI{300}{\kilo\meter} wide and \SI{30}{\kilo\meter} high.     \TODO{we want to consider much higher resolutions, too} The mountain profile has the same form as equation~\ref{eqn:resting:mountain}, but the gravity waves tests have a mountain height of $h_0 = \SI{250}{\meter}$.  As in the resting atmosphere test, $a = \SI{5}{\kilo\meter}$ is the mountain half-width and $\lambda = \SI{4}{\kilo\meter}$ is the wavelength.  On the optimised SLEVE grid, $s_1 = \SI{5}{\kilo\meter}$ is the large scale height, $s_2 = \SI{2}{\kilo\meter}$ is the small scale height and the optimal exponent value $n = 1.35$ is used.
...
Test results are compared between the BTF, SLEVE and cut cell grids at several resolutions.
...
At $\Delta z = \SI{300}{\meter}$, vertical velocities on the BTF, SLEVE and cut cell grids are visually indistinguishable (not shown).  They agree with the high resolution mass-conserving semi-implicit semi-Lagrangian solution from \citet{melvin2010}.  \TODO{perhaps I should spot check some of the grids at higher resolutions but, given that the theta fields converge overall, I don't see why vertical velocities contours would do anything funky}
...
Results are similar across all grid types and resolutions, but examining the anomalies in the lee of the mountain errors are found in the lowest few layers: the bottommost layer is anomalously warm and the layer above it is anomalously cold.  \TODO{this isn't true at all resolutions, e.g. 50dz is cold at the bottom}  At all resolutions, the amplitude of the errors is greater on the cut cell grid than the BTF \TODO{or SLEVE?} grid.  As resolution increases, the anomalously cold layers first increase in amplitude, then begin to span multiple layers, which suggests that the errors are controlled by the mountain height. \TODO{that conclusion might be stronger with tests at different mountain heights?  oh no, not more tests! :-P}
