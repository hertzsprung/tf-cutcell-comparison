...
In order to create a semi-implicit formulation, \cite{weller-shahrokhi2014} split \(H\) into diagonal and off-diagonal components and the off-diagonal components are treated explicitly in the Helmholtz equation.  However, this was found to be unstable when using a large timestep on a grid with large non-orthogonality.  Improved stability is achieved by splitting \(H\) into the orthogonal part and the non-orthogonal correction: 
\begin{align}
