...
To summarize, results of the gravity waves test on all grids are in good agreement with the reference solution from \citet{melvin2010}.  Stripes of potential temperature anomalies are found in the lee of the mountain, and these errors are most severe on the cut cell grid.
...
\begin{table}
	\caption{\TODO{DOUBLE-CHECK THESE VALUES.}}
	\label{tab:gw-meshes}
%
\centering
...
{$\Delta z$ (\si{\meter})} & \multicolumn{2}{c}{max/min cell area ratio} & \multicolumn{2}{c}{Nonorthogonality (\si{\degree})} \\
 & {BTF} & {Cut cell} & {BTF} & {Cut cell} \\
\hline
500	& 1.01 &  999  & 999    & 999 \\
...
%
	\caption{Differences in potential temperature between the start and end of the gravity waves test with $\Delta z = \SI{50}{\meter}$ \TODO{if we have the results in time!} showing (a) waves in potential temperature on the BTF grid.  Enlargements of the same domain near the mountain on (b) the BTF grid, and (c) the cut cell grid \TODO{revealing errors near the ground in the lee of the mountain}.  The gravity waves test results are compared with thermal advection test results, showing differences in tracer density between the numeric and analytic solutions at $t = \SI{18000}{\second}$ on (d) the BTF grid, and (e) the cut cell grid, in which errors are seen which are similar to those in (c).  Axes are in units of \si{\meter}.}
	\label{fig:gw-theta}
\end{figure*}
