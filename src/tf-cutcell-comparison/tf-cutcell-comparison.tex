...
To summarize, results of the gravity waves test on all grids are in good agreement with the reference solution from \citet{melvin2010}.  The potential temperature field converges with increasing resolution, though errors are found in the lowest layers on the cut cell grids.
...
%
	\caption{Differences in potential temperature between the start and end of the gravity waves test on the BTF grid with $\Delta z = \SI{50}{\meter}$ \TODO{if we have the results in time!}  Axes are in units of \si{\meter}.}
	\label{fig:gw-theta}
\end{figure*}
