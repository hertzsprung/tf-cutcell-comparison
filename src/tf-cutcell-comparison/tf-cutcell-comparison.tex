...
The model is operated in a two-dimensional $x$--$z$ slice configuration without Coriolis forces.  The model uses the C-grid staggering in the horizontal and the Lorenz staggering in the vertical such that $\theta$, $\rho$ and $\Pi$ are stored at cell centres and the covariant component of velocity at cell faces.
