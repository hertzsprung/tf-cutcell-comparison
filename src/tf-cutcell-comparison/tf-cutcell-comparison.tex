...
The OpenFOAM utility \texttt{snappyHexMesh} was used to create grids that approximate the cut cell method.
...
The grid that \texttt{snappyHexMesh} accepts as input is generated by a custom utility so that \texttt{snappyHexMesh} performs fewer, smaller vertex displacements, resulting in grids that are closer to those produced by typical shaving methods, as described by \citet{adcroft1997}.
...
The scheme uses a least squares fit of a multidimensional polynomial using an upwind-biased stencil.  Weightings are iteratively adjusted in the least squares fit procedure to ensure that advection remains upwind-biased where the stencil includes small cut cells.  \TODO{I can see reviewers saying that this isn't a sufficiently detailed explanation...?}
...
Errors are small on the BTF grids with a maximum error in tracer density of \num{0.0376} at $\Delta z = \SI{500}{\meter}$ and errors are 10 to 100 times smaller on higher resolution BTF grids.
...
Enlargements of the error field near the mountain are shown in figure~\ref{fig:thermalAdvection} at $\Delta z = \SI{50}{\meter}$.
...
\begin{table}
	\caption{Cell area ratios and nonorthogonality of BTF and cut cell grids used in the gravity waves and thermal advection tests.  Cell sizes are almost uniform on BTF grids, but for the cut cell grids the cell area ratio gives an indication of the smallest cell sizes.  Nonorthogonality is the angle between the surface normal vector and the vector joining the two adjacent cell centres.  Except at coarse resolutions, cut cell grids have greater maximum nonorthogonality than BTF grids.}
	\label{tab:gw-meshes}
%
\centering
\footnotesize
...
{$\Delta z$ (\si{\meter})} & \multicolumn{2}{c}{max/min cell area ratio} & \multicolumn{2}{c}{Maximum nonorthogonality (\si{\degree})} \\
 & {BTF} & {Cut cell} & {BTF} & {Cut cell} \\
\hline
500	& 1.01 &  1.68  & 9.2   & 4.1 \\
