...
% Errors in calculating the horizontal pressure gradient generate spurious circulations and so much effort has been put into improving this calculation.  This is often achieved by making the grid more orthogonal, flattening terrain following coordinate surfaces [citations] or using a cut cell approach [citations].
...
% In a test of orographically induced gravity waves, we find that the advection of potential temperature generates grid-scale errors on the cut cell grid.  The errors persist because the Lorenz staggering of variables renders them invisible to the model. [TODO: I don't think we have enough evidence to support this final sentence]
...
\TODO{make it clearer that we're trying to determine if non-orthogonality or misalignment of the flow is the primary source of error}
...
% APPENDICES
\appendix[A]
\appendixtitle{Non-orthogonal formulation}
